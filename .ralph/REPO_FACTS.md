...
- TURN-TRACE GREEN PROOF (2026-09-24): the exact stored verifyCommand exits 0 against CI run 35714886605 for HEAD 38f71a8 (run conclusion success, remote main == HEAD so ci_verify.sh reused it with no push): 12 matched tests all pass — TurnTraceStoreTest 7/7 + TurnTraceProductionWiringTest 5/5, 0 failures. OwnerBiometricBindingTest 9/9 also passes (the Sep-22 "regression" was a phantom). The only full-suite failures in that run are the known pre-existing baselines StorePersistenceTest x1 + TermuxJarvisServerTest x1, filtered out by the class-level filter. An auto-reject whose /tmp/ralph_verify_*.txt reads `ci_verify.sh: GITHUB_TOKEN is not set` is a HARNESS env failure, never a code failure — reproduce by hand with the token set before debugging.

## 2026-09-24 — THREAD-OBJECTS CLOSED (Gate 3a priority 2)
- `com.jarvis.app.threads.ThreadObjects` (object) + `ThreadTracker`: pure in-memory Kotlin, no I/O/network/model — the engine seam is `CognitiveEngine.threadTracker: ThreadTracker?` (default null, appended after turnTraceStore). `ThreadObjects.segment_message` splits a message into thoughts on punctuation boundaries (script-agnostic: `.`/`!`/`?` + Arabic `؟`/`؛` + ellipsis; dots/`...` variants normalized to `…`); `completenessOf` marks a thought TRAILING_OFF iff it ends on an ellipsis or a dangling conjunct (`and`/`w`/`و`/`كمان`); `significantTokens` (lowercase, len>2, stopwords incl. `bas fel el al ana enta kda`, keeps digit-letter Franco-Arabic like `3ayez`/`5alas`) drives the `isRelatedTopic` (shared token) and `isIdleMoment` (≤1 token) gates. `ThreadTracker.ingestTurn` runs `close_detect` FIRST then creates `thread-{turn}-{i}` objects (i=0 = main thread, its own mainAnchorId; every tangent points mainAnchorId at it). `thread_acknowledge` = one clause per thread born this turn (`noted: …` FINISHED / `noted open: … (held open, not completed)` TRAILING_OFF). `resurface_policy` returns an open TRAILING_OFF thread only inside its decay window (minimumHoldTurns..maximumOpenTurns AND minimumHoldMs..maximumOpenMs with injectable `now`) AND on an idle/related turn — the clock alone never fires. `close_detect` closes open threads sharing a topic token with a turn carrying any RESOLUTION_MARKERS member (`done sorted resolved forgot never mind nevermind cancelled canceled 5alas khalas khallas bas khallas zabat zabatna 3amalt 3aleet shelna`). AC7 lever: `setEnabled(false)` makes the whole tracker inert.
- ENGINE WIRING GOTCHA: `CognitiveEngine` calls `ingestTurn(cognitiveResult.turnIndex, …)` on `turnIndex: Long` from `turnCounter.incrementAndGet()` (so the FIRST turn is index 1 — production-test thread ids are `thread-1-*` and `mainAnchorId(1L)`, and a resurfaced thread prints `(open from turn 1)`). Kotlin does not widen Int→Long, so any future seam keyed on turnIndex must match type exactly.
- DECAY-CLOCK vs TEST TIME: the production tracker default `minimumHoldMs = 60_000` blocks resurfacing in test-speed turns. Production-path tests must inject `ThreadTracker(minimumHoldMs = 0)`; unit tests should inject `now = { … }` or 0 ms. Story fixtures keep defaults in JarvisEngine.init production composition.
- `[Open threads]` block: appended to the FULL generation payload (`fullContextMessage = userText + galaxyContext + identitySuffix + buildThreadContext(...)`) so the bridge-captured text carries it. Layout: `\n\n[Open threads]:\n- <clause>` per clause, ack clauses in creation order then `returning: {content} (open from turn {createdAtTurn})` per resurfaced thread. Empty string when tracker null/disabled/nothing to say (AC7: the reply stays byte-identical).
- THIRD-STORY fixture family (all three languages, low-doubt safe — no `it`/`that`/`this` substrings, no VAGUE_ACTION verbs): THREE_THOUGHTS = `Green tea smells pleasant. بابا راح يشترى نضارة. ana kent bas faaker fel safar ...` (→ FINISHED/FINISHED/TRAILING_OFF); turn 2 `El safar gamed awy` resurfaces thread-1-2 (shares `safar`); turn 3 `Zabatna el safar` close_detects it (marker `zabatna`). Prove each fixture stays low-doubt by scanning for the banned substrings before committing.
- HARDENING NOTE: `significantTokens` trims `\u2026` since an attached ellipsis (`safar…` — no preceding space) would otherwise glue to the token and break `isRelatedTopic` vs a later plain `safar`.
- COMPILE GOTCHA (first CI, run 35998661856): `OpenThread`/`Completeness` are NESTED inside `object ThreadObjects`; a sibling top-level class in the SAME package (`ThreadTracker`, same file) does NOT auto-scope them — Kotlin requires an explicit `import com.jarvis.app.threads.ThreadObjects.OpenThread` (+ Completeness) even for same-package/same-file siblings. Unresolved `OpenThread` cascades into 30+ errors (map inference, `.copy` args, buildList type-inference at the CALLER CognitiveEngine:585) — diagnose via the FIRST `e: ` line in the job-logs download, never the tail. GREEN PROOF (2026-09-24): run 35999731772 for HEAD aff9030 — stored verifyCommand matched 44 tests (threads.* + ThreadObjectsProductionWiringTest + SystemGraphTest), all passed, EXIT=0.

## 2026-09-24 — SIGNAL-SPLIT CLOSED (Gate 3a priority 3)
- `com.jarvis.app.memory.SignalSplitScorer` (object-free: `SignalProfile` + `SignalSplitScorer` + `MemoryNode.withSignals(profile)`): six DISTINCT per-record signals — relevance (binary-quantized Hamming similarity to context, same mechanism as MemoryImportanceScorer.relevanceScore/KNN), importance (saturates UP with repetitionCount via 0.6+0.4*sat(r) when the fact is an explicit user directive; NEVER decays — 5x-stated boundary keeps 0.85 importance on the 5th mention), uncertainty (hedge markers -> 0.8, declarative -> 0.15), novelty (1/(1+r), the surprisal axis that DOES decay), consent (explicit-directive markers `never/please/i want/...` -> 1.0 else 0.0), cost (0f — no consumer computes it). `MemoryNode` carries all six as nullable default-null fields; `computeProfile(node, ctx, repetitionCount)` NEVER returns a single score (SignalProfile is 6 floats only — a combined blend is a later baseline-gated story). `repetitionCount` derives from STORED EVIDENCE — e.g. `store.getHistory(subject,predicate).size` over an N-call supersession chain (the AC3 fixture: addFact x5 -> 5-node chain -> 5th mention keeps top-tier >=0.8).
- PRD-premise grounding: the Stage-04 note ("no prediction-error scalar exists") is REAL; SIGNAL-SPLIT was grounded at the storage layer (MemoryNode carried only salience/accessCount pre-split). Do not re-search for an FEP scorer to satisfy this story family.
- Negative-control pattern: the naive single prediction-error scalar lives INSIDE the test (`SinglePredictionErrorScorer`, importance == surprisal == 1/(1+r)) and FAILS AC3's own fixture (0.167 < 0.8 on the 5th mention) while the split scorer PASSES the same node + same repetitionCount (0.85 >= 0.8) — one fixture, two models, opposite verdicts.
- No CognitiveEngine/JarvisEngine/SystemGraph wiring landed with SIGNAL-SPLIT (AC4: no consumer may blend the six yet). Any later story that consumes them must first prove a combined score beats a simple baseline (most-recent-N or keyword-match).
- GREEN PROOF (2026-09-24): run 36005534332 for HEAD fb2e051 — stored verifyCommand (one-file no-network grep + `ci_verify.sh 'com.jarvis.app.memory.SignalSplitScorerTest.*'`) matched 6/6 tests, all passed, EXIT=0 on the FIRST push. Whole-module compile succeeded, so the MemoryNode data-class extension broke no legacy construction (audit: all 15 construction sites use named args).

## 2026-09-26 — CORRECTION-CHAIN CLOSED (Gate 3b priority 4)
- GREEN PROOF: CI run **36231351087** for HEAD `6a9dfd8` — stored verifyCommand (`.ralph/ci_verify.sh 'com.jarvis.app.memory.CorrectionChainTest.*' '*SystemGraphTest*'`) matched 35 tests, all passed, EXIT=0. Full `junit-results` artifact for the same run: **1257 tests / 0 failures / 0 errors / 0 skipped** (was 1233 before this story; 24 new tests). `ci_verify.sh` only greps the FILTERED tests, so the full-suite failure count must be checked separately from the `junit-results` artifact (`GET /repos/mhw77655-hue/Friday/actions/runs/<run>/artifacts` → the `junit-results` zip) or a story can pass its filter while the suite is red.
- Compile-error triage without local Gradle: `GET /repos/mhw77655-hue/Friday/actions/jobs/<job_id>/logs` returns the job log as PLAIN TEXT (not a zip — `unzip` fails, read it directly); a compile failure shows up as `> Task :mobile:app:compileDebugKotlin FAILED` followed by `e: file:///home/runner/work/...` lines, and the job list comes from `.../actions/runs/<run_id>/jobs`.
- `android.database.sqlite.Cursor` DOES NOT EXIST in the Android SDK stubs (it is the removed legacy subclass). `SQLiteDatabase.rawQuery` returns `android.database.Cursor` — import THAT type for the nullable column readers in `AndroidMemoryGraphStore`. (Two CI iterations were spent on the same 12 cascading `Unresolved reference` errors; the first error line names the import itself, so read line 1 of the error list, not the last.)
- `TermuxJarvisServerTest` rebinds the hardcoded port 18081 six times in one class (setUp/tearDown per test), and the kernel releases the listening socket only a few ms after `ServerSocket.close()` — every OTHER test failed with `BindException: Address already in use` in 0.001–0.004s. `TermuxJarvisServer.start()` now binds through `bindWithReleaseRetry` (`BIND_ATTEMPTS = 40` × `BIND_RETRY_MS = 50ms`, then rethrows the real failure). This is also the correct behaviour for a production restart on fixed port 8081; the test class itself was never modified.
- `MemoryGraphStore.addFact` now returns the inserted node id (`String`) and closes every currently-valid node sharing subject+predicate with `validUntil` + a forward `supersededBy` pointer. Every `MemoryNode` construction site (16) uses named args; the six SignalProfile axes stay separately stored and are NEVER blended into a combined score.
- `MemoryAccessibility` (object in `SignalSplitScorer.kt`: `initial()`, `STEP`, `raise(current, passes)`, `MemoryNode.withRaisedAccessibility()`) is the accessibility-only seam: `ConsolidationDaemon` moves accessibility and reports `accessibilityReinforced`; it can physically not reach a stored signal through that path, which is what AC3's byte-identity assertion pins. `AndroidMemoryGraphStore` migrates schema v1→v2 (nullable `supersededBy`/`accessibility`/signal columns).
- `MemoryCorrection` (new file) recognises correction markers only in LEADING position (English + Egyptian Arabic) and treats `not sure`/`no idea`/`not certain` as NOT corrections; `CognitiveEngine`'s live write-back stores the STRIPPED assertion with source `live-correction`. Corpus-level mirror of the first-minimal implementation rule (one occurrence = an accident, three = a pattern): English/Arabic correction markers appeared exactly 3× each in JARVIS_BIBLE/, prompt guardrails and the provenance test corpus.
- `JarvisOrganGraph` gained `memory.consolidationDaemon → memory.graphStore` SENDS_TO (alongside `cognitive.engine → memory.graphStore`), pinned by `SystemGraphTest` so the chain path is a production dependency edge and not test-only wiring.

## 2026-09-26 — ADAPTER-MANIFEST CLOSED (Gate 3b priority 5, last story of the stage)
- CONTRACT ONLY: no adapter training exists in this codebase and none was added. `com.jarvis.app.memory.provenance.AdapterManifest` (interface, extends `AdapterLoadGate`) + `JsonlAdapterManifest` — register/state/sources/taint/retrainPlan/mayLoad/registrations over an append-only local JSON-Lines file at `filesDir/memory/adapters.jsonl`. Ids only, never adapter bytes and never memory content, so the file is byte-scan safe. `AdapterState.ACTIVE|TAINTED`.
- The load seam IS real: `ModelManager.adapterLoadGate: AdapterLoadGate? = null` (default null → byte-for-byte pre-story behaviour), consulted at the TOP of `loadOnce` — the single point `wake`/`request` funnels through — BEFORE `beginLoad`, so a refused adapter takes no cache slot and cannot slip in via the single-flight path. `JarvisEngine.init` constructs the manifest BEFORE the ModelManager and hands it over; `TermuxJarvisServer` mirrors the param.
- Adapter↔memory linkage goes through the EXISTING provenance walk, not a parallel index: `register()` mirrors every batch into the ProvenanceLedger as `ProvenanceKind.ADAPTER_BATCH`, and `MemoryForgetter`'s ADAPTER_BATCH branch calls `adapterManifest.taint(...)`. `retrainPlan` = registered sources minus taintedBy minus anything `TombstoneStore.isTombstoned`, so a plan can never re-introduce an already-forgotten memory. Organ `memory.adapterManifest` (REAL MEMORY) with `memory.forgetPropagation SENDS_TO` it and `model.modelManager READS_FROM` it, pinned by SystemGraphTest.
- `ModelProviderType` is declared in **`com.jarvis.app.env`** (env/EnvironmentProfile.kt), NOT `com.jarvis.app.model` — even though every user of it sits in the model package. Importing it from `com.jarvis.app.model` is the natural mistake and resolves nowhere.
- A WORKFLOW conclusion of `success` proves NOTHING about tests here: `testDebugUnitTest` is `continue-on-error:true`, and when the test COMPILE fails the `junit-results` artifact is simply ABSENT. Verify in this order: runs API → `.../actions/runs/<id>/artifacts` (junit-results present?) → `.../actions/runs/<id>/jobs` → `.../actions/jobs/<job_id>/logs` (plain text, NOT a zip) → grep `^e: ` and read the FIRST error line; everything after it is collateral.
- ModelManager caches ONE handle per TIER, so any test asserting a load refusal (or loading two different models through one manager) must construct a FRESH ModelManager per scenario — otherwise the second load is served from cache and never reaches the load path.
- GREEN PROOF: run **36247528448** for HEAD `4fdb934` — stored verifyCommand matched 50 testcases (provenance.* + SystemGraphTest), all passed, EXIT=0, and the full `junit-results` artifact reports **1262 tests / 0 failures / 0 errors / 0 skipped** (was 1257). The whole module is green for the first time in this stage.
- All five Gate-3b stories (BASELINE-GREEN, PROVENANCE-LEDGER, FORGET-PROPAGATION, CORRECTION-CHAIN, ADAPTER-MANIFEST) are `passes: true`; the Stage-3 PRD has no remaining story.
